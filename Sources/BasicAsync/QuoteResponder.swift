...
{
    let quoteRepository: ThreadSpecificVariable<QuoteRepository> // = ThreadSpecificVariable<QuoteRepository>() //FIXME: Hidden Dependency
    
    func respond(to request: HTTPRequest) -> EventLoopFuture<HTTPResponse> {
        switch request.head.method {
        case .GET:
            guard request.head.uri != "/" else { return listQuotes(for: request) }
            ...
            return quote(id: id, for: request)
        case .POST:
    ...
    func makeQuoteRepository(for request: HTTPRequest) -> QuoteRepository {
        if let existingQuoteRepository = quoteRepository.currentValue { return existingQuoteRepository }
    ...
    func createQuote(from request: HTTPRequest) -> EventLoopFuture<HTTPResponse> {
        ...
        catch { return request.eventLoop.newFailedFuture(error: error) }
    }
    
    func quote(id: String, for request: HTTPRequest) -> EventLoopFuture<HTTPResponse> {
    ...
    func deleteQuote(by id: String, for request: HTTPRequest) -> EventLoopFuture<HTTPResponse> {
            guard let id = UUID(uuidString: id) else {
                return request.eventLoop.newFailedFuture(error: QuoteAPIError.invalidIdentifier)
            }
            let repository = makeQuoteRepository(for: request) //FIXME: Imposter Constructor
